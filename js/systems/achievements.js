// achievements.js — Achievement tracking with fixed triggers
(function() {
    'use strict';

    const ACHIEVEMENTS = [
        { id: 'first_jump',    name: 'First Air',        desc: 'Perform your first jump',         check: (G) => G.distance > 50 },
        { id: 'first_flip',    name: 'Spin Doctor',      desc: 'Land a flip trick',                check: (G) => G.comboCount > 0 },
        ...
        { id: 'triple_flip',   name: 'Triple Threat',    desc: 'Land a triple flip (1080°)',       check: (G) => G.trickList && G.trickList.some(t => t.text && t.text.includes('1080')) }
    ];
    ...
    window.AchievementSystem = {
        ...
        check: function(trigger, value) {
            ...
            for (const ach of ACHIEVEMENTS) {
                ...
                if (ach.check(G)) {
                    ...
                    try { localStorage.setItem('snowAchievements', JSON.stringify(G.achievementsEarned)); } catch(e) {}
                }
            }
        },
        ...
        showPopup: function(ach) {
            ...
            G._achPopups.push({ text: `🏆 ${ach.name}`, sub: ach.desc, life: 2.5 });
        },

        getList: function() { return ACHIEVEMENTS; }
    };
})();
